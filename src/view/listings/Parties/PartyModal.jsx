...
function PartyModal({ show, handleClose, isEdit, partyToEdit }) {
  ...
  const [amount, setAmount] = useState("");
  const [date, setDate] = useState(new Date());
  const [limitType, setLimitType] = useState("no");
  const [creditLimit, setCreditLimit] = useState("");
  const [isEditingAddress, setIsEditingAddress] = useState(false);
  const [shippingAddress, setShippingAddress] = useState("");
  const [billingAddress, setBillingAddress] = useState("");
  ...
  const handleGstTypeChange = (e) => {
    ...
    setGstType(newId);
    const selectedGst = gstTypes.find(type => String(type.id) === newId);
    setGstTypeName(selectedGst ? selectedGst.name : "");
  };
  ...
  useEffect(() => {
    if (isEdit && partyToEdit) {
      ...
      setLimitType(partyToEdit.limitType || "no");
      setSelectedState(partyToEdit.state || "");
      setGstType(partyToEdit.gstTypeId || "");
      setGstTypeName(partyToEdit.gstTypeName || "");
    } else {
      resetFields();
    }
  }, [isEdit, partyToEdit]);
  ...
  const savePartyLogic = async () => {
    const newParty = {
      ...
      ...additionalFields
        .filter(f => f.isChecked)
        .reduce((acc, f, idx) => {
          acc[`custom_field_${idx + 1}_name`] = f.name;
          acc[`custom_field_${idx + 1}_value`] = f.value;
          return acc;
        }, {}),
    };
  ...
  const handleUpdate = async () => {
    if (!partyToEdit?.parties_id) return;
    const updatedParty = {
      ...partyToEdit,
      name: partyName,
      gstin,
  ...
  const handleDelete = async () => {
    if (!partyToEdit?.parties_id) return;
    if (!window.confirm(`Are you sure you want to delete ${partyToEdit.name}?`)) return;
    try {
      await dispatch(deleteExistingParty(partyToEdit.parties_id)).unwrap();
      handleClose();
      resetFields();
  ...
  return (
    <Modal show={show} onHide={handleClose} size="lg" centered>
      ...
      <Modal.Body>
        ...
        <Row className="mb-3">
          <Col>
            <Form.Control type="text" placeholder="Party Name *" value={partyName} onChange={e => setPartyName(e.target.value)} />
          </Col>
          <Col>
            <Form.Control type="text" placeholder="GSTIN" value={gstin} onChange={e => setGstin(e.target.value)} />
          </Col>
          <Col>
        ...
        <Nav variant="tabs" activeKey={activeTab} onSelect={setActiveTab} className="mb-3">
          <Nav.Item><Nav.Link eventKey="gst">GST & Address</Nav.Link></Nav.Item>
          <Nav.Item><Nav.Link eventKey="credit">Credit & Balance</Nav.Link></Nav.Item>
          <Nav.Item><Nav.Link eventKey="additional">Additional Fields</Nav.Link></Nav.Item>
        </Nav>
        ...
        {activeTab === "gst" && (
          <Row>
            <Col md={4}>
              <Form.Group className="mb-3">
                <Form.Label>GST Type</Form.Label>
                <Form.Select value={gstType} onChange={handleGstTypeChange}>
                  <option value="">Select GST Type</option>
                  {gstTypes.map(type => <option key={type.id} value={type.id}>{type.name}</option>)}
                </Form.Select>
              </Form.Group>
              <Form.Group className="mb-3">
                <Form.Label>Select State</Form.Label>
            ...
            <Col md={4}>
              <Form.Group className="mb-3">
                <Form.Label>Billing Address</Form.Label>
                <Form.Control as="textarea" rows={3} placeholder="Billing Address" value={billingAddress} onChange={e => setBillingAddress(e.target.value)} />
              </Form.Group>
            </Col>
            <Col md={4}>
              <Form.Label>Shipping Address</Form.Label>
              <div>
                {isEditingAddress ? (
                  <>
                    <Form.Control as="textarea" rows={3} placeholder="Enter address" value={shippingAddress} onChange={e => setShippingAddress(e.target.value)} autoFocus />
                    <div className="text-end mt-2">
                      <Button variant="primary" size="sm" style={{ backgroundColor: "#4a93dcff", border: "none" }} onClick={() => setIsEditingAddress(false)}>Save</Button>
                    </div>
                  </>
                ) : (
                  <Button variant="link" className="p-0 text-primary" onClick={() => setIsEditingAddress(true)}>{shippingAddress || "+ Add Address"}</Button>
                )}
              </div>
            </Col>
          </Row>
        ...
        {activeTab === "credit" && (
          <Row>
            <Col md={4}>
              <Form.Group className="mb-3">
                <Form.Control type="number" placeholder="Opening Balance" value={amount} onChange={e => setAmount(e.target.value)} />
              </Form.Group>
            </Col>
            <Col md={4}>
              <Form.Group className="mb-3">
                <DatePicker selected={date} onChange={setDate} className="form-control" dateFormat="dd/MM/yyyy" placeholderText="As of Date" />
              </Form.Group>
            </Col>
            <Col md={4}>
              <Form.Group className="mb-3">
                <div className="d-flex position-relative mt-2" style={{ width: "290px", borderRadius: "50px", padding: "2px", justifyContent: "space-between", backgroundColor: "#e9f3ff" }}>
                  <div className="position-absolute bg-primary" style={{ width: "130px", height: "85%", borderRadius: "50px", transition: "transform 0.3s", transform: limitType === "no" ? "translateX(0%)" : "translateX(calc(100% + 24px))" }}></div>
                  <Button variant="transparent" className={`flex-grow-1 ${limitType === "no" ? "text-white" : "text-primary"}`} onClick={() => setLimitType("no")} style={{ zIndex: 1, borderRadius: "50px", width: "130px" }}>No Limit</Button>
                  <div className="px-1" style={{ display: "flex", alignItems: "center", color: "#0d6efd", fontSize: "1.2rem", zIndex: 1 }}>⇄</div>
                  <Button variant="transparent" className={`flex-grow-1 ${limitType === "custom" ? "text-white" : "text-primary"}`} onClick={() => setLimitType("custom")} style={{ zIndex: 1, borderRadius: "50px", width: "130px" }}>Custom Limit</Button>
                </div>
                {limitType === "custom" && <Form.Control type="number" placeholder="Enter Credit Limit" value={creditLimit} onChange={e => setCreditLimit(e.target.value)} className="mt-3" />}
              </Form.Group>
            </Col>
          </Row>
        )}
          ...
          <Row>
            <Col md={6}>
              {additionalFields.map(field => (
                <Form.Group key={field.id} className="mb-3 d-flex align-items-center">
                  <div className="d-flex align-items-center" style={{ width: field.isChecked ? '50%' : '100%' }}>
                    <Form.Check type="checkbox" className="me-2" checked={field.isChecked} onChange={e => handleAdditionalFieldChange(field.id, 'isChecked', e.target.checked)} />
                    <Form.Control type="text" placeholder={field.name} value={field.name} onChange={e => handleAdditionalFieldChange(field.id, 'name', e.target.value)} className="flex-grow-1" />
                  </div>
                  {field.isChecked && <div className="ms-3" style={{ width: '50%' }}><Form.Control type="text" placeholder={`Enter value for ${field.name}`} value={field.value} onChange={e => handleAdditionalFieldChange(field.id, 'value', e.target.value)} /></div>}
                </Form.Group>
              ))}
              <Form.Group className="mb-3 d-flex align-items-center">
                <Form.Check type="checkbox" className="me-2" />
                <DatePicker selected={date} onChange={setDate} className="form-control" dateFormat="dd/MM/yyyy" placeholderText="Select Date" />
              </Form.Group>
            </Col>
          </Row>
        )}
      ...
      <Modal.Footer>
        {isEdit ? (
          <>
            <Button variant="danger" className="px-4 py-2" onClick={handleDelete}>Delete</Button>
            <Button variant="primary bg-primary" className="text-white px-4 py-2" onClick={handleUpdate}>Update</Button>
          </>
        ) : (
          <>
            <Button variant="outline-primary" onClick={handleSaveAndNew}>Save & New</Button>
            <Button variant="primary bg-primary" className="text-white px-4 py-2" onClick={handleSave}>Save</Button>
          </>
        )}
      </Modal.Footer>
    </Modal>

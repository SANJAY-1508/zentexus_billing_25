...
// Function to generate simple reference number like Ref-3, Ref-4
const generateReferenceNo = (existingProforma = []) => {
  if (existingProforma.length === 0) {
    return "Ref-1";
  }
  
  // Extract all numbers from reference_no fields
  const numbers = existingProforma.map(p => {
    if (!p.reference_no) return 0;
    
    // Try to extract number from formats like "Ref-3", "Ref-1", etc.
    const match = p.reference_no.match(/Ref-(\d+)/);
...
const ProformaCreation = ({ tabNumber = 1 }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    billing_address: "",
    shipping_address: "",
    reference_no: "Ref-1", // Default starting number
...
const addNewTab = () => {
  ...
  // Generate reference number for new tab
  const referenceNo = generateReferenceNo(proforma);
...
useEffect(() => {
  ...
  useEffect(() => {
    if (isCreateMode) {
      // Generate reference number immediately
      const referenceNo = generateReferenceNo(proforma);
      setFormData((prev) => ({
        ...prev,
        reference_no: referenceNo,
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      reference_no: proformaToEdit.reference_no || "Ref-1",
      invoice_date:
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // 2. Final payload - INCLUDE the reference_no generated by frontend
      const payload = {
        ...formData,
        reference_no: formData.reference_no, // Send the frontend-generated reference number
        products: JSON.stringify(formData.rows),

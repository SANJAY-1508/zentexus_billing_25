...
// // Add this helper function to extract numeric values from reference numbers
// const extractNumericValue = (refString) => {
//   if (!refString) return NaN;
  
//   // Remove any non-numeric characters and get the number
//   const match = refString.toString().match(/\d+/);
...
//     billing_address: "",
//     shipping_address: "",
//     reference_no: "", // CHANGED: invoice_no -> reference_no
...
//   // Get all reference numbers from ALL tabs
//   const allReferenceNumbers = [];
  
//   // 1. Add reference numbers from all stored tabs
//   Object.values(tabForms).forEach(tabForm => {
//     if (tabForm.reference_no && tabForm.reference_no !== "Generating...") {
//       const refNum = extractNumericValue(tabForm.reference_no);
//       if (!isNaN(refNum)) {
//         allReferenceNumbers.push(refNum);
//       }
//     }
//   });
  
//   // 2. Add current active tab's reference number
//   if (formData.reference_no && formData.reference_no !== "Generating...") {
//     const refNum = extractNumericValue(formData.reference_no);
//     if (!isNaN(refNum)) {
//       allReferenceNumbers.push(refNum);
//     }
//   }
...
//     const maxRef = Math.max(...allReferenceNumbers);
//     nextReferenceNumber = maxRef + 1;
//   }
...
//     billing_address: "",
//     shipping_address: "",
//     reference_no: nextReferenceNumber.toString(), // Use calculated number
//     invoice_date: new Date().toISOString().split("T")[0],
//     state_of_supply: "",
//     payment_type: "",
//     description: "",
...
//   // Generate reference number in create mode
//   useEffect(() => {
//     if (isCreateMode) {
//       dispatch(searchSales("")).then((action) => {
//         if (action.payload && action.payload.length > 0) {
//           // Get all existing reference numbers from sales
//           const allSales = action.payload;
//           const allReferenceNumbers = [];
          
//           allSales.forEach(sale => {
//             // Try to get reference_no first, then invoice_no as fallback
//             let refNumber = sale.reference_no || sale.invoice_no || "";
            
//             if (refNumber) {
//               const numericValue = extractNumericValue(refNumber);
//               if (!isNaN(numericValue)) {
//                 allReferenceNumbers.push(numericValue);
//               }
//             }
//           });
...
//           let nextNum = 1;
//           if (allReferenceNumbers.length > 0) {
//             nextNum = Math.max(...allReferenceNumbers) + 1;
//           }

//           setFormData((prev) => ({
//             ...prev,
//             reference_no: nextNum.toString(),
//           }));
...
//     // Extract reference number from existing data
//     let referenceNumber = saleToEdit.reference_no || saleToEdit.invoice_no || "";
//     if (referenceNumber) {
//       // Extract numeric part from any format (e.g., "INV202512-0022" -> "22")
//       const numericMatch = referenceNumber.toString().match(/\d+/);
//       referenceNumber = numericMatch ? numericMatch[0] : "";
//     }
...
//       reference_no: referenceNumber, // CHANGED: Use extracted number
//       invoice_date:
...
//         billing_address: saleToEdit.billing_address || "",
//         shipping_address: saleToEdit.shipping_address || "",
//         reference_no: referenceNumber, // CHANGED
...
//       const payload = {
//         ...formData,
//         reference_no: formData.reference_no, // CHANGED: Send reference_no
//         products: JSON.stringify(formData.rows),
...
//       NotifyData(err || "Failed to Save Proforma", "error");
//     }
//   };
...
//                 <TextInputform
//                   formLabel="Reference No" // CHANGED: Invoice No -> Reference No
//                   value={formData.reference_no || "Generating..."} // CHANGED
//                   readOnly={true}
//                   style={{
...
//                   <Button variant="danger" onClick={handleSave}>  Save Proforma</Button>
...
const ProformaCreation = ({ tabNumber = 1 }) => {
  ...
  const location = useLocation();
  const [selectedPartyOption, setSelectedPartyOption] = useState(null);
  const { parties, partiesStatus, proforma } = useSelector((state) => state.proforma);
  ...
  const [isManualRoundOff, setIsManualRoundOff] = useState(false);
  const proformaToEdit = id ? proforma.find((p) => p.proforma_id == id) : null;
   const fromEstimate = location.state?.fromEstimate;
   const estimateData = location.state?.estimateData;
  const [formData, setFormData] = useState({
    parties_id: "",
...
useEffect(() => {
  if (fromEstimate && estimateData) {
    ...
    // Success notification
    NotifyData(`Estimate #${estimateData.estimate_no} loaded for conversion to Proforma`, "success");
  ...
  useEffect(() => {
    if (isCreateMode) {
      dispatch(searchProforma("")).then((action) => {
        if (action.payload && action.payload.length > 0) {
          // Get all existing reference numbers from proforma
          const allProforma = action.payload;
          const allReferenceNumbers = [];
          
          allProforma.forEach(proforma => {
            // Try to get reference_no first, then invoice_no as fallback
            let refNumber = proforma.reference_no || proforma.invoice_no || "";
          ...
          // No proforma exist, start from 1
          setFormData((prev) => ({
            ...prev,
            reference_no: "1",
          }));
  ...
  useEffect(() => {
    ...
    const totalAmountRaw = rows.reduce((a, r) => a + Number(r.amount || 0), 0);
    const rount_off = Number(proformaToEdit.rount_off || 0);
    ...
    // Extract reference number from existing data
    let referenceNumber = proformaToEdit.reference_no || proformaToEdit.invoice_no || "";
    if (referenceNumber) {
      // Extract numeric part from any format (e.g., "INV202512-0022" -> "22")
      const numericMatch = referenceNumber.toString().match(/\d+/);
      referenceNumber = numericMatch ? numericMatch[0] : "";
    ...
    setFormData({
      ...
      reference_no: referenceNumber, // CHANGED: Use extracted number
      invoice_date:
        proformaToEdit.invoice_date || new Date().toISOString().split("T")[0],
    ...
    setTabForms(prev => ({
      ...
      1: {
        ...
        shipping_address: proformaToEdit.shipping_address || "",
        reference_no: referenceNumber, // CHANGED
        invoice_date: proformaToEdit.invoice_date || new Date().toISOString().split("T")[0],
        state_of_supply: proformaToEdit.state_of_supply || "",
        payment_type: proformaToEdit.payment_type || "",
        description: proformaToEdit.description || "",
        add_image: proformaToEdit.add_image || "",
  ...
  if (isCreateMode || fromEstimate) {
    ...
    dispatch(createProforma(payload)); // Redux use பண்ணுறேனா இது
    // அல்லது
    // await axiosInstance.post("/proforma.php", payload);
  }
};
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // VERY IMPORTANT: Only add edit_proforma_id in EDIT mode
      if (isEditMode) {
                      ...
                      {imagePreview && (
                        <Button
                          ...
                          onClick={() => {
                            ...
                            document.getElementById("proforma-image-upload").value = "";
                          }}
                        >
                          X
                        </Button>

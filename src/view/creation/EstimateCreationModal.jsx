...
const EstimateCreation = ({ tabNumber = 1 }) => {
  ...
  // const { parties, partiesStatus, estimates } = useSelector((state) => state.estimate);
  const { parties = [], partiesStatus = "idle", estimates = [] } = useSelector(
  ...
  //   (state) => state.product
  // );
  const { products = [], status: productStatus = "idle" } = useSelector(
  (state) => state.product || {}
);
  // const { units, status: unitStatus } = useSelector((state) => state.unit);
  const { units = [], status: unitStatus = "idle" } = useSelector((state) => state.unit || {});
  ...
  const [formData, setFormData] = useState({
    ...
    billing_address: "",
    shipping_address: "",
    estimate_no: "", // Changed from invoice_no to estimate_no
    estimate_date: new Date().toISOString().split("T")[0], // Changed from invoice_date
    state_of_supply: "",
    payment_type: "",
    description: "",
    add_image: "",
  ...
  const addNewTab = () => {
    ...
    // Get all estimate numbers from ALL tabs
    ...
    const newFormData = {
      ...
      billing_address: "",
      shipping_address: "",
      estimate_no: nextEstimateNo, // Changed from invoice_no
      estimate_date: new Date().toISOString().split("T")[0], // Changed from invoice_date
      state_of_supply: "",
      payment_type: "",
      description: "",
    ...
    // Update tabs - Changed tab title from "Sale#" to "Estimate#"
    setTabs(prev => [
  ...
  useEffect(() => {
    ...
    const rount_off = Number(estimateToEdit.rount_off || 0);
    ...
    setTabForms(prev => ({
      ...
      1: {
        ...
        shipping_address: estimateToEdit.shipping_address || "",
        estimate_no: estimateToEdit.estimate_no || "", // Changed from invoice_no
        estimate_date: estimateToEdit.estimate_date || new Date().toISOString().split("T")[0], // Changed from invoice_date
        state_of_supply: estimateToEdit.state_of_supply || "",
        payment_type: estimateToEdit.payment_type || "",
        description: estimateToEdit.description || "",
        add_image: estimateToEdit.add_image || "",
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        estimate_no: formData.estimate_no, // Changed from invoice_no
      ...
      // IMPORTANT: Changed from edit_sales_id to edit_estimates_id
      if (isEditMode) {
      ...
      dispatch(searchEstimates(""));
      navigate("/estimate"); // Changed from /sale to /estimate
    } catch (err) {
      console.error("Save error:", err);
      NotifyData(err || "Failed to save estimate", "error");
    }
  };

  const handleBack = () => navigate("/estimate"); // Changed from /Sale to /estimate
            ...
            <Row className="additional-actions mt-3 align-items-center">
              ...
              <Row className="mt-3">
                <Col xs={3}>
                  ...
                  <label
                    htmlFor="estimate-image-upload" // Changed from sale-image-upload
                    style={{
                  ...
                  {!isViewMode && (
                    <>
                      <input
                        id="estimate-image-upload" // Changed from sale-image-upload
                        type="file"
                        accept="image/*"
                      ...
                      {imagePreview && (
                        <Button
                          ...
                          onClick={() => {
                            ...
                            document.getElementById("estimate-image-upload").value = "";
                          }}
                        >
                          X
                        </Button>
